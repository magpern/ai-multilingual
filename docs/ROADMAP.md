...
**Post-v1.4.0 program:** Multilingual SEO & Localized URLs — [MSEO_PARENT_IMPLEMENTATION_PLAN.md](plans/MSEO_PARENT_IMPLEMENTATION_PLAN.md) (**Architecture Frozen**; [ADR-0023](adr/0023-localized-url-overlay-architecture.md); MSEO.0 **complete**; MSEO.1 **complete** — [closure](plans/MSEO1_CLOSURE.md); MSEO.2 plan **FREEZE** — [MSEO2 plan](plans/MSEO2_PUBLIC_LOCALIZED_URL_ROUTING_SEO_GRAPH_IMPLEMENTATION_PLAN.md); MSEO.2 implementation / MSEO.3–MSEO.5 remain; **MSEO.3 NOT STARTED**). STATE B / `Migrator::TARGET` **8**.
