...
| **F12** Limited rollout | **Architecture frozen** — [canonical plan](plans/STRATEGY_F_F12_LIMITED_ROLLOUT.md); PO operational values pending. Operational only (cohort/flags, telemetry, metrics, performance, caching, diagnostics, rollback). **No** new translator features. **Implementation not started.** |
