...
**Current next decision:** **OTL.5** Bounded Bulk Operations — planning freeze / Architecture Frozen lifecycle ([`plans/OTL5_BOUNDED_BULK_OPERATIONS_IMPLEMENTATION_PLAN.md`](plans/OTL5_BOUNDED_BULK_OPERATIONS_IMPLEMENTATION_PLAN.md)). Do **not** implement OTL.5 production code until the plan is frozen on main and the combined implementation task begins. Do not start OTL.6 or TSC under OTL. **OTL.0–OTL.4 Complete**. OTL parent Architecture Frozen ([`plans/OTL_PARENT_IMPLEMENTATION_PLAN.md`](plans/OTL_PARENT_IMPLEMENTATION_PLAN.md); freeze merge `9a31176f0147d726b251315259cd6d6ca84ea432`). **v1.2.0 released** (tag `v1.2.0` → `b67fc296e2b2170dea84228b1acda502e518f07a`). **TQ.0–TI.7 Complete**. **TIQ Complete.** Runtime `Migrator::TARGET` **7**.
...
| Order | Program | Focus |
|---|---|---|
| 1 | **OTL — Operator Translation Lifecycle** | Coherent operator workflow over TIQ services. Parent: [`plans/OTL_PARENT_IMPLEMENTATION_PLAN.md`](plans/OTL_PARENT_IMPLEMENTATION_PLAN.md). **OTL.0–OTL.4 Complete.** **OTL.5** Bounded Bulk Operations — Architecture Frozen planning ([`plans/OTL5_BOUNDED_BULK_OPERATIONS_IMPLEMENTATION_PLAN.md`](plans/OTL5_BOUNDED_BULK_OPERATIONS_IMPLEMENTATION_PLAN.md)); production implementation not started. |
