...
**Current next decision:** Begin authorized **TSC.0** implementation from frozen main via `feature/tsc0-*` when an implementation task is opened. **Do not** start TSC.1. **TIQ Complete.** **OTL Complete.** **v1.3.0 released** (tag `v1.3.0`). Runtime `Migrator::TARGET` **7**. TSC production architecture is **site-neutral**.
...
Ladder: **TSC.0 → TSC.1 → TSC.2 → TSC.3 → TSC.4 → TSC.5 → TSC.6**. Site-neutral surface coverage around the existing Store. **STATE A / TARGET 7.** **TSC.0** plan **Architecture Frozen** on `main` — [`TSC0_INTERNAL_SURFACE_CAPABILITY_FOUNDATION_IMPLEMENTATION_PLAN.md`](plans/TSC0_INTERNAL_SURFACE_CAPABILITY_FOUNDATION_IMPLEMENTATION_PLAN.md). Implementation **not started.**
...
| **TSC.0 Internal Surface Capability Foundation** | [`plans/TSC0_INTERNAL_SURFACE_CAPABILITY_FOUNDATION_IMPLEMENTATION_PLAN.md`](plans/TSC0_INTERNAL_SURFACE_CAPABILITY_FOUNDATION_IMPLEMENTATION_PLAN.md) (**Architecture Frozen** on `main`; implementation not started) |
