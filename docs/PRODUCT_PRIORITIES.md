...
**Current next decision:** **OTL.4** Jobs Integration — [plan](plans/OTL4_JOBS_INTEGRATION_IMPLEMENTATION_PLAN.md) (**Architecture Frozen** on `main`; freeze merge `aaacaf3d6bacb2547ff41c53b46a9000a15d7ebd`). Production implementation next. Do not start OTL.5–OTL.6 or TSC under OTL. **OTL.0–OTL.3 Complete**. OTL parent Architecture Frozen ([`plans/OTL_PARENT_IMPLEMENTATION_PLAN.md`](plans/OTL_PARENT_IMPLEMENTATION_PLAN.md); freeze merge `9a31176f0147d726b251315259cd6d6ca84ea432`). **v1.2.0 released** (tag `v1.2.0` → `b67fc296e2b2170dea84228b1acda502e518f07a`). **TQ.0–TI.7 Complete**. **TIQ Complete.** Runtime `Migrator::TARGET` **7**.
...
| Order | Program | Focus |
|---|---|---|
| 1 | **OTL — Operator Translation Lifecycle** | Coherent operator workflow over TIQ services. Parent: [`plans/OTL_PARENT_IMPLEMENTATION_PLAN.md`](plans/OTL_PARENT_IMPLEMENTATION_PLAN.md). **OTL.0–OTL.3 Complete.** **OTL.4** Architecture Frozen ([`plans/OTL4_JOBS_INTEGRATION_IMPLEMENTATION_PLAN.md`](plans/OTL4_JOBS_INTEGRATION_IMPLEMENTATION_PLAN.md)); production implementation not started. |
