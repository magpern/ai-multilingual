...
**Current next decision:** **OTL.1** Operations list + attention — **planning only** (OTL.0 **Complete** on `main`; merge `13e68f9d51ca5a4a0a8704ed048cf51e3eec3d3a`; plan [`plans/OTL0_FOUNDATIONS_IMPLEMENTATION_PLAN.md`](plans/OTL0_FOUNDATIONS_IMPLEMENTATION_PLAN.md)). Do not implement OTL.1 until its plan is independently reviewed and frozen on `main`. OTL parent Architecture Frozen ([`plans/OTL_PARENT_IMPLEMENTATION_PLAN.md`](plans/OTL_PARENT_IMPLEMENTATION_PLAN.md); freeze merge `9a31176f0147d726b251315259cd6d6ca84ea432`). **v1.2.0 released** (tag `v1.2.0` → `b67fc296e2b2170dea84228b1acda502e518f07a`). **TQ.0–TI.7 Complete**. **TIQ Complete.** Runtime `Migrator::TARGET` **7**.
...
| Order | Program | Focus |
|---|---|---|
| 1 | **OTL — Operator Translation Lifecycle** | Coherent operator workflow over TIQ services. Parent: [`plans/OTL_PARENT_IMPLEMENTATION_PLAN.md`](plans/OTL_PARENT_IMPLEMENTATION_PLAN.md). **OTL.0 Complete.** Next: **OTL.1** planning only — Operations list + attention. |
