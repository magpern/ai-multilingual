...
### Translation Surface Coverage (TSC) — TSC.0 Complete

Authoritative parent: [TSC_PARENT_IMPLEMENTATION_PLAN.md](plans/TSC_PARENT_IMPLEMENTATION_PLAN.md).

Ladder: **TSC.0 → TSC.1 → TSC.2 → TSC.3 → TSC.4 → TSC.5 → TSC.6**. Site-neutral surface coverage around the existing Store. **STATE A / TARGET 7.** **TSC.0 Complete** on `main` — [`TSC0_INTERNAL_SURFACE_CAPABILITY_FOUNDATION_IMPLEMENTATION_PLAN.md`](plans/TSC0_INTERNAL_SURFACE_CAPABILITY_FOUNDATION_IMPLEMENTATION_PLAN.md). **TSC.1–TSC.6** not started. Next: definitive **TSC.1** milestone planning.
