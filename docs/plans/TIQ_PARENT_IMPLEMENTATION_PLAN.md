...
**TI.6:** planning freeze in progress — [TI6_JOBS_SCALE_SAFETY_POLISH_IMPLEMENTATION_PLAN.md](TI6_JOBS_SCALE_SAFETY_POLISH_IMPLEMENTATION_PLAN.md) (Architecture Frozen planning — implementation **not started**)
**TI.7:** planning/implementation **not started**
**Next:** After TI.6 planning freeze merges to `main`, implement TI.6 on `feature/ti6-jobs-scale-safety-polish` only. Do **not** start TI.7 until TI.6 is Complete and TI.7’s plan is Architecture Frozen.
...
| TI.6 planning | **In progress** — [TI6_JOBS_SCALE_SAFETY_POLISH_IMPLEMENTATION_PLAN.md](TI6_JOBS_SCALE_SAFETY_POLISH_IMPLEMENTATION_PLAN.md); implementation not started |
| TI.6 implementation | Only after this plan is Architecture Frozen on `main` |
