...
**TI.7:** definitive planning **in progress** on `docs/ti7-controlled-auto-publication-policy-plan` — [plan](TI7_CONTROLLED_AUTO_PUBLICATION_POLICY_IMPLEMENTATION_PLAN.md) (architecture-review candidate); [ADR-0020](../adr/0020-controlled-auto-publication-and-frontend-gate.md) (**Proposed**). Implementation **not started**. Runtime `TARGET` remains **6**.
**Next:** Complete ADR-0020 acceptance + independent TI.7 planning review; Architecture Freeze on `main`; then implement TI.7. Do **not** start TI.7 production coding until the plan is Architecture Frozen.
