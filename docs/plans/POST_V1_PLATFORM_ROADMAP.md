...
- **Operator Translation Lifecycle (OTL)** — [parent plan](OTL_PARENT_IMPLEMENTATION_PLAN.md) (**Architecture Frozen** on `main`; freeze merge `9a31176f0147d726b251315259cd6d6ca84ea432`; OTL.0–OTL.6). **OTL.0 Complete**; **OTL.1 Complete**; **OTL.2 Complete**; **OTL.3 Complete** on `main` (merge `77fc39da5d9b30d204e5a0c04e318a463ad39484`). **OTL.4** Architecture Frozen — [OTL4_JOBS_INTEGRATION_IMPLEMENTATION_PLAN.md](OTL4_JOBS_INTEGRATION_IMPLEMENTATION_PLAN.md) (production implementation not started). `Migrator::TARGET` **7**; tag `v1.2.0` released.
