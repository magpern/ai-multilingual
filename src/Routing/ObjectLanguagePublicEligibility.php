...
final class ObjectLanguagePublicEligibility {
	...
	public function is_route_publishable( WP_Post $post, int $language_id ) {
		...
		if ( null === $lang || Languages::STATUS_PUBLISHED !== (string) ( $lang->status ?? '' ) ) {
			return new WP_Error( 'aiml_slug_language_not_published', __( 'Language is not published.', 'universal-multilingual' ) );
		}

		if ( ! in_array( (string) $post->post_status, array( 'publish', 'private' ), true ) ) {
			return new WP_Error( 'aiml_slug_source_not_public', __( 'Source object is not publicly available.', 'universal-multilingual' ) );
		}

		if ( ! $this->capabilities->supports_post( $post ) ) {
			return new WP_Error( 'aiml_slug_capability_unsupported', __( 'Routing capability does not support this object.', 'universal-multilingual' ) );
		}

		if ( ! $this->has_overlay_bundle( Store::SOURCE_POST, (int) $post->ID, $language_id, Extractor::FIELD_SLUG ) ) {
			return new WP_Error( 'aiml_slug_overlay_bundle_empty', __( 'Object/language has no overlay-eligible segments.', 'universal-multilingual' ) );
		}
	...
	public function is_term_route_publishable( WP_Term $term, int $language_id ) {
		...
		if ( ! $this->capabilities->supports_term_object( $term ) ) {
			return new WP_Error( 'aiml_slug_capability_unsupported', __( 'Routing capability does not support this taxonomy.', 'universal-multilingual' ) );
		}

		if ( ! $this->has_overlay_bundle( Store::SOURCE_TERM, (int) $term->term_id, $language_id, TermExtractor::FIELD_SLUG ) ) {
			return new WP_Error( 'aiml_slug_overlay_bundle_empty', __( 'Term/language has no overlay-eligible segments.', 'universal-multilingual' ) );
		}
