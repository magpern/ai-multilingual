<?php
/**
 * Spike S5 — Strategy E evaluated against the required operation matrix.
 *
 * Identity: Strategy C. Reconciliation: Strategy E (displaced-row rematch).
 * Render: StrategyERenderGate — rendered_false_positive must be 0.
 *
...
final class StrategyEOperationsTest extends \WP_UnitTestCase {

	private static array $all_results = array();

	public static function tearDownAfterClass(): void {
		file_put_contents(
			__DIR__ . '/../corpus/strategy-e-results.json',
		...
		if ( array() !== $failures ) {
			self::fail(
				'Strategy E exit gate: rendered_false_positive must be 0. Failed cases: ' . implode( ', ', $failures )
			);
		}
	...
	private function evaluate( OracleTree $tree, callable $op ): array {
		...
		$keys_before     = count( StrategyE::extract( $before_content ) );
		...
		$keys_after     = count( StrategyE::extract( $after_content ) );
	...
	public function test_type_conversion_in_place(): void {
		...
		$before_keys = array_keys( StrategyE::extract( $tree->to_content() ) );

		$result = $this->evaluate( $tree, static function ( OracleTree $t ) use ( $a ) {
			$t->convert_type( $a->id, 'core/heading', array( 'level' => 2 ) );
		} );

		$after_keys = array_keys( StrategyE::extract( $result['after_content'] ) );
	...
	public function test_identical_text_same_block_type_distinct_paths(): void {
		...
		$keys = array_keys( StrategyE::extract( $tree->to_content() ) );
	...
	public function test_path_reuse_after_delete_then_insert_two_saves(): void {
		...
		$tree->delete( $old->id );
		$sync = StrategyEReconciler::sync_source( $rows, StrategyE::extract( $tree->to_content() ) );
		$rows = $sync['rows'];

		$new = Builders::paragraph( $ids, 'Different content at reused path zero.' );
		$tree->insert( null, 0, $new );
		$after_segments = StrategyE::extract( $tree->to_content() );
	...
	public function test_delete_save_restore_save_two_saves(): void {
		...
		$sync    = StrategyEReconciler::sync_source(
			array(
				$key => array(
					'block_name'      => $before[ $key ]['block_name'],
					'source_hash'     => ReconciliationSimulator::source_hash( $before[ $key ]['text'] ),
			...
			StrategyE::extract( $tree->to_content() )
		);
		$rows = $sync['rows'];
