<?php
/**
 * Spike S5 — Strategy F evaluated against the required operation matrix.
 *
 * Identity: persistent UUID (b:<uuid>:content). Reconciliation: UUID direct match.
 * Render: StrategyFRenderGate — rendered_false_positive must be 0.
 *
...
use AIMultilingual\Spike\S5\Strategy\StrategyFUuidSync;

final class StrategyFOperationsTest extends \WP_UnitTestCase {

	private static array $all_results = array();

	public static function tearDownAfterClass(): void {
		file_put_contents(
			__DIR__ . '/../corpus/strategy-f-results.json',
			wp_json_encode( self::$all_results, JSON_PRETTY_PRINT )
		);

		$failures = array();
		foreach ( self::$all_results as $case => $metrics ) {
			if ( ( $metrics['rendered_false_positive'] ?? 0 ) > 0 ) {
				$failures[] = $case;
			}
		}
		...
		if ( array() !== $failures ) {
			self::fail(
				'Strategy F exit gate: rendered_false_positive must be 0. Failed cases: ' . implode( ', ', $failures )
			);
		}
	...
	private function record( string $case, array $result, array $extra = array() ): void {
		...
		self::$all_results[ $case ] = array_merge(
			...
			array(
				...
				'gate_results'         => $result['gate_results'] ?? array(),
				'inject_stats_after'   => $result['inject_stats_after'] ?? array(),
				'content_mutations'    => $result['metrics']['content_mutations'] ?? 0,
			),
			$extra
		);
	}
	...
	private function evaluate( OracleTree $tree, callable $op ): array {
		...
		$before_prep       = StrategyF::prepare( $before_content );
		$keys_before       = count( $before_prep['segments'] );
		...
		$after_prep     = StrategyF::prepare( $after_content );
		$keys_after     = count( $after_prep['segments'] );
		...
		$result['key_collision_before'] = false;
		$result['key_collision_after']  = false;
	...
	public function test_type_conversion_in_place(): void {
		...
		$before_keys = array_keys( StrategyF::extract( $tree->to_content() ) );

		$result = $this->evaluate( $tree, static function ( OracleTree $t ) use ( $a ) {
			$t->convert_type( $a->id, 'core/heading', array( 'level' => 2 ) );
		} );

		$after_keys = array_keys( StrategyF::extract( $result['after_content'] ) );

		$this->record( 'type_conversion_in_place', $result, array(
			'before_keys' => $before_keys,
			'after_keys'  => $after_keys,
			'observed'    => 'convert_type replaces attrs and drops aimlBlockId unless merged — new UUID assigned',
		) );
	...
	public function test_identical_text_same_block_type_distinct_paths(): void {
		...
		$keys = array_keys( StrategyF::extract( $tree->to_content() ) );
	...
	public function test_path_reuse_after_delete_then_insert_two_saves(): void {
		...
		$before = StrategyF::extract( $tree->to_content() );
		StrategyFUuidSync::apply( $tree, StrategyF::prepare( $tree->to_content() )['content'] );
		...
		$tree->delete( $old->id );
		$sync = StrategyFReconciler::sync_source( $rows, StrategyF::extract( $tree->to_content() ) );
		$rows = $sync['rows'];

		$new = Builders::paragraph( $ids, 'Different content at reused path zero.' );
		$tree->insert( null, 0, $new );
		$after_segments = StrategyF::extract( $tree->to_content() );
		$sync           = StrategyFReconciler::sync_source( $rows, $after_segments );
		$rows           = $sync['rows'];

		$reconciled_key = array_key_first( $after_segments );
		$row            = $rows[ $reconciled_key ] ?? null;
		$gate           = StrategyFRenderGate::resolve(
			$reconciled_key,
			$row,
			$after_segments[ $reconciled_key ],
			array()
		);

		self::$all_results['path_reuse_delete_then_insert_two_saves'] = array(
			'old_key'              => $key,
			'new_key'              => $reconciled_key,
			'same_key_as_before'   => $reconciled_key === $key,
			'old_row_status'       => $rows[ $key ]['status'] ?? null,
			'renders'              => $gate['renders'],
			'gate_reason'          => $gate['reason'],
			'observed'             => 'new block gets new UUID; old row orphaned',
		);

		$this->assertNotSame( $key, $reconciled_key );
		$this->assertSame( ReconciliationSimulator::STATUS_IGNORED, $rows[ $key ]['status'] ?? '' );
	...
	public function test_dynamic_block_skipped(): void {
		...
		$segments = StrategyF::extract( $content );
		...
		$this->assertMatchesRegularExpression( '/^b:[0-9a-f-]+:content$/', (string) array_key_first( $segments ) );
	}
	...
	public function test_reusable_block_reference_skipped(): void {
		...
		foreach ( array_keys( $segments ) as $seg_key ) {
			$this->assertMatchesRegularExpression( '/^b:[0-9a-f-]+:content$/', $seg_key );
		}
	}
	...
	public function test_delete_save_restore_save_two_saves(): void {
		...
		StrategyFUuidSync::apply( $tree, $prep['content'] );
		$before  = $prep['segments'];
		$key     = array_key_first( $before );
		$removed = $tree->delete( $b->id );
		$this->assertSame(
			$before[ $key ]['uuid'],
			$removed->attrs[ \AIMultilingual\Spike\S5\Strategy\StrategyFContract::ATTR_NAME ] ?? null
		);
		$sync    = StrategyFReconciler::sync_source(
			array(
				$key => array(
					'block_name'      => $before[ $key ]['block_name'],
					'source_hash'     => ReconciliationSimulator::source_hash( $before[ $key ]['text'] ),
			...
			StrategyF::extract( $tree->to_content() )
		);
		$rows = $sync['rows'];

		$tree->restore( null, 0, $removed );
		$after_prep = StrategyF::prepare( $tree->to_content() );
		StrategyFUuidSync::apply( $tree, $after_prep['content'] );
		$this->assertArrayHasKey( $key, $after_prep['segments'] );
		$sync         = StrategyFReconciler::sync_source( $rows, $after_prep['segments'] );
		$rows         = $sync['rows'];

		self::$all_results['delete_save_restore_save_two_saves'] = array(
			'row_status'  => $rows[ $key ]['status'] ?? null,
			'restored'    => $sync['stats']['restored'] ?? 0,
			'renders'     => isset( $rows[ $key ] ) && null !== ReconciliationSimulator::translated_value( $rows[ $key ] ),
			'preserved'   => $sync['stats']['preserved'] ?? 0,
			'observed'    => 'restore with same UUID reattaches reviewed row',
		);

		$this->assertSame( 'reviewed', $rows[ $key ]['status'] ?? '' );
		$this->assertSame( 1, $sync['stats']['restored'] ?? 0 );
	}
