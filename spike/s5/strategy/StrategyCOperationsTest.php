<?php
/**
 * Spike S5 — Strategy C evaluated against the required operation matrix.
 *
 * Key shape: b:<structural_path>:<block_name>:content
 * per docs/plans/AI_MULTILINGUAL_PLANNING.md. Observed metrics recorded in
 * strategy-c-results.json; conclusions drawn from evidence, not baked-in guesses.
 *
...
final class StrategyCOperationsTest extends \WP_UnitTestCase {

	private static array $all_results = array();

	public static function tearDownAfterClass(): void {
		file_put_contents(
			__DIR__ . '/../corpus/strategy-c-results.json',
	...
	private function evaluate( OracleTree $tree, callable $op ): array {
		...
		$keys_before     = count( StrategyC::extract( $before_content ) );

		$result = StrategyEvaluator::evaluate( $tree, $op, array( StrategyC::class, 'extract' ) );

		$after_content  = $result['after_content'];
		$eligible_after = count( RealBlockWalker::walk_eligible( $after_content ) );
		$keys_after     = count( StrategyC::extract( $after_content ) );
	...
	public function test_type_conversion_in_place(): void {
		...
		$before_keys = array_keys( StrategyC::extract( $tree->to_content() ) );

		$result = $this->evaluate( $tree, static function ( OracleTree $t ) use ( $a ) {
			$t->convert_type( $a->id, 'core/heading', array( 'level' => 2 ) );
		} );

		$after_keys = array_keys( StrategyC::extract( $result['after_content'] ) );
	...
	public function test_move_between_containers(): void {
		...
		$this->record( 'move_between_containers', $result, array(
			'observed' => 'path prefix changes on cross-container move → orphaned + spurious_new',
		) );
	...
	public function test_wrap_block(): void {
		...
		$this->record( 'wrap_block', $result, array(
			'observed' => 'leaf path 0→0.0 on wrap → orphaned + spurious_new',
		) );
		$this->assertSame( 1, $result['metrics']['orphaned'] );
		$this->assertSame( 1, $result['metrics']['spurious_new'] );
	}
	...
	// -- Duplicate / copy — distinct paths, but trailing block path shifts. --
	...
	public function test_identical_text_same_block_type_distinct_paths(): void {
		...
		$keys = array_keys( StrategyC::extract( $tree->to_content() ) );
	...
	public function test_path_reuse_after_delete_then_insert_two_saves(): void {
		...
		$tree->delete( $old->id );
		$rows = ReconciliationSimulator::sync_source( $rows, StrategyC::extract( $tree->to_content() ) );

		$new = Builders::paragraph( $ids, 'Different content at reused path zero.' );
		$tree->insert( null, 0, $new );
		$after_segments = StrategyC::extract( $tree->to_content() );
		$rows           = ReconciliationSimulator::sync_source( $rows, $after_segments );
	...
	public function test_delete_save_restore_save_two_saves(): void {
		...
		self::$all_results['delete_save_restore_save_two_saves'] = array(
			'row_status' => $rows[ $key ]['status'],
			'renders'    => null !== ReconciliationSimulator::translated_value( $rows[ $key ] ),
			'observed'   => 'un-orphaning gap — same as A/B',
		);
