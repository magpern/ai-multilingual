/**
 * Gutenberg editor interactions. THROWAWAY — spike/s5 only.
 *
 * ROOT-CAUSE FIX (Phase 3): modern Gutenberg (WP >= 6.3) renders the block
 * list inside `iframe[name="editor-canvas"]`. Block toolbars, the "Options"
 * (three-dot) menu, and the Save button render in the *main* document,
 * positioned over the iframe by coordinate translation. Every block-content
 * locator MUST go through `canvas()`; every toolbar/menu/save locator MUST
 * use `page` directly. The Phase 2 harness used `page.locator(...)` for
 * block content, which matched zero elements and timed out — this was an
 * automation defect, not a Gutenberg incompatibility.
 */
...
export async function login(page: Page, creds: WpCredentials): Promise<void> {
  const cookieFile = path.join(path.resolve(__dirname, '../../corpus/browser-validation/auth-cookies.json'));
